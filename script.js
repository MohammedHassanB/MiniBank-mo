...
const account1 = {
  ...
  movementsDates: [
    ...
    '2024-06-13T17:01:17.194Z',
    '2024-06-15T23:36:17.929Z',
...
const formatMovsDate=function(date,local)
{
  const clacPassedDayes=(date1,date2)=>Math.round(Math.abs(date2-date1)/(1000*60*60*24));
  
  
    const daysPassed=clacPassedDayes(new Date(),date);
...
const formatCur=function(val,local,currency){
...
const displayMovements = function (acc, sort = false) {
  ...
  movs.forEach(function (mov, i) {
    ...
    const displayDate=formatMovsDate(curdate,acc.local);
...
const calcDisplaySummary = function (acc) {
  ...
  const out = acc.movements
    .filter(mov => mov < 0)
    .reduce((acc, mov) => acc + mov, 0);
  labelSumOut.textContent = formatCur(-1*out,acc.local,acc.currency);
